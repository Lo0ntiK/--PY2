...
class PaperBook(Book):
    ...
    def pages(self, set_pages: int) -> None:
        ...
        if set_pages <= 0:
            raise ValueError('Чило страниц больше нуля')
        self._pages = set_pages

    def __str__(self):
        return f"Бумажная книга {self.name}. Автор {self.author}. Число страниц {self.pages}"
...
class AudioBook(Book):
    ...
    def duration(self, set_duration: float) -> None:
        if not isinstance(set_duration, float):
            raise TypeError('Продолжительность - число с плавающей запятой')
        if set_duration <= 0:
            raise ValueError('Продолжительность больше нуля')
